...
const Materials: React.FC = () => {
  ...
  const EXTERNAL_SOURCES = [
    {
      name: 'PCI Concursos',
      description: 'A maior fonte de provas anteriores e apostilas gratuitas do Brasil.',
      url: 'https://www.pciconcursos.com.br/provas/',
      category: 'Provas & Apostilas'
    },
    {
      name: 'Gran Cursos Gratuitos',
      description: 'E-books, editais verticalizados e cursos completos para baixar.',
      url: 'https://www.grancursosonline.com.br/materiais-gratuitos',
      category: 'E-books'
    },
    {
      name: 'Estratégia (Grátis)',
      description: 'Materiais de alta qualidade, PDFs e videoaulas gratuitas.',
      url: 'https://www.estrategiaconcursos.com.br/blog/materiais-gratuitos/',
      category: 'Cursos'
    },
    {
      name: 'Gabarite',
      description: 'Apostilas completas em PDF organizadas por matérias.',
      url: 'https://gabarite.com.br/material-concurso/',
      category: 'Apostilas'
    },
    {
      name: 'Apostilas Opção',
      description: 'Conteúdo gratuito focado em diversos concursos públicos.',
      url: 'https://www.apostilasopcao.com.br/apostilas-gratuitas',
      category: 'Apostilas'
    }
  ];
  ...
  const handleGenerateMore = async () => {
    setGenerating(true);
    try {
      // Tentar obter a rotina para saber o perfil do usuário
      const routine = await getStudyRoutine();
      const studyType = routine?.studyType || 'concurso';
      
      const newMaterials = await generateStudyMaterials(3, studyType);
  ...
  const handleCreateCustom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newMaterial = await createCustomMaterial(customTopic, studyType);
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      const content = await generateMaterialContent(selectedMaterial, studyType);
  ...
  const handleExtendContent = async () => {
    ...
    try {
      ...
      const additionalContent = await extendMaterialContent(selectedMaterial, selectedMaterial.content, studyType);
      ...
      setTimeout(() => {
        const view = document.getElementById('material-content-view');
        if (view) {
    ...
    <div className="space-y-6 relative no-print">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-700 dark:text-slate-100">Materiais de Estudo</h1>
            <p className="text-slate-400 dark:text-slate-500 font-bold">Apostilas da IA e fontes gratuitas.</p>
          </div>
          {materials.length > 0 && activeTab === 'mine' && (
            <button 
      ...
      {/* Tabs */}
      <div className="flex gap-4 border-b-2 border-slate-200 dark:border-slate-700 pb-px">
        <button
        ...
        >
          Minhas Apostilas
          {activeTab === 'mine' && (
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-blue-600 dark:bg-blue-400 rounded-t-full shadow-[0_-2px_10px_rgba(37,99,235,0.3)]" />
          )}
        </button>
        <button
        ...
        >
          Biblioteca Gratuita
          {activeTab === 'free' && (
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-emerald-600 dark:text-emerald-400 rounded-t-full shadow-[0_-2px_10px_rgba(16,185,129,0.3)]" />
          )}
        </button>
      </div>
      ...
      {activeTab === 'mine' ? (
        <>
          ...
          {loading && materials.length === 0 ? (
              ...
              {filteredMaterials.map((material) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="p-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-blue-500">
                  ...
                  <h3 className="text-lg font-black text-slate-700 dark:text-slate-100 mb-2 line-clamp-2 leading-tight group-hover:text-blue-600 transition-colors uppercase">
                    {material.title}
                  </h3>
                  <p className="text-slate-400 dark:text-slate-500 text-sm font-bold line-clamp-2 mb-4">
                    {material.summary}
                  </p>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-12 text-center border-2 border-slate-200 dark:border-slate-700">
              ...
              <h3 className="text-2xl font-black text-slate-700 dark:text-slate-100 mb-2 uppercase tracking-tight">Sua estante está vazia</h3>
              <p className="text-slate-400 dark:text-slate-500 font-bold mb-8 max-w-sm mx-auto">
                Peça uma apostila personalizada ou use as sugestões da IA para começar a estudar.
              </p>
              <button 
                onClick={handleGenerateMore}
                className="bg-blue-600 text-white px-8 py-4 rounded-2xl border-b-4 border-blue-800 font-bold uppercase tracking-widest hover:bg-blue-500 active:border-b-0 active:translate-y-1 transition-all"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
          {EXTERNAL_SOURCES.map((source, idx) => (
            <a 
              key={idx}
              href={source.url}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-white dark:bg-slate-800 rounded-3xl p-6 border-2 border-slate-200 dark:border-slate-700 hover:border-emerald-400 dark:hover:border-emerald-500 transition-all cursor-pointer group relative overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 flex flex-col"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="p-3 bg-slate-100 dark:bg-slate-700 rounded-2xl group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30 transition-colors">
                  <Download size={24} className="text-emerald-500" strokeWidth={2.5} />
                </div>
                <span className="text-[10px] font-black uppercase tracking-tighter px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full border border-emerald-200 dark:border-emerald-800/30">
                  {source.category}
                </span>
              </div>
              
              <h3 className="text-xl font-black text-slate-700 dark:text-slate-100 mb-2 leading-tight group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                {source.name}
              </h3>
              
              <p className="text-slate-400 dark:text-slate-500 text-sm font-bold mb-4 flex-grow">
                {source.description}
              </p>
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100 dark:border-slate-700">
                ...
                <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                  <PlusCircle size={18} className="-rotate-90" strokeWidth={3} />
                </div>
              </div>
            </a>
          ))}
            ...
            <div id="material-content-view" className="flex-grow overflow-y-auto p-4 md:p-12 bg-slate-50 dark:bg-slate-900 print:bg-white print:p-0 print:overflow-visible">
              {selectedMaterial.content ? (
                <div className="prose dark:prose-invert prose-slate prose-lg max-w-none bg-white dark:bg-slate-800 p-8 md:p-12 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-sm print:border-none print:shadow-none print:p-0">
                   ...
                   {/* Botão de Estender Conteúdo */}
                   <div className="mt-12 pt-8 border-t-2 border-slate-100 dark:border-slate-700 no-print">
                      <div className="text-center space-y-4">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full text-sm font-black uppercase tracking-widest">
                          <Sparkles size={16} />
                          Fim desta parte
                        </div>
                        <h4 className="text-xl font-black text-slate-700 dark:text-slate-100">Quer aprender ainda mais?</h4>
                        <p className="text-slate-500 dark:text-slate-400 font-bold max-w-md mx-auto">
                          O BizuBot pode continuar escrevendo esta apostila, trazendo detalhes ainda mais profundos e novos exercícios.
                        </p>
                        
                        <button
                          onClick={handleExtendContent}
                          disabled={extendingContent}
                          className="mt-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all flex items-center gap-3 mx-auto shadow-xl disabled:opacity-50"
                        >
                          {extendingContent ? (
                            <>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center no-print">
                   ...
                   <p className="text-slate-400 dark:text-slate-500 font-bold mb-8 max-w-md">
                     O BizuBot vai escrever uma apostila completa e detalhada, com teoria aprofundada, cálculos passo a passo e exercícios com gabarito.
                   </p>
