...
const Schedule: React.FC = () => {
  ...
  const [studyType, setStudyType] = useState<'concurso' | 'academico'>('concurso');

  useEffect(() => {
    loadRoutine();

    const handleFocus = () => loadRoutine(true);
    const handleVisibility = () => {
      if (!document.hidden) loadRoutine(true);
    };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const newRoutine = await generateStudyRoutine(exam, hours, subjects, studyType);
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border-2 border-slate-200 dark:border-slate-700 shadow-sm">
          <form onSubmit={handleCreate} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <button
                type="button"
                onClick={() => setStudyType('concurso')}
                className={`py-4 rounded-2xl font-bold border-2 transition-all ${
                  studyType === 'concurso'
                    ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-400 text-blue-600 dark:text-blue-400 shadow-inner'
              ...
              <button
                type="button"
                onClick={() => setStudyType('academico')}
                className={`py-4 rounded-2xl font-bold border-2 transition-all ${
                  studyType === 'academico'
                    ? 'bg-purple-50 dark:bg-purple-900/20 border-purple-400 text-purple-600 dark:text-purple-400 shadow-inner'
                    : 'bg-slate-50 dark:bg-slate-700 border-slate-100 dark:border-slate-600 text-slate-400 dark:text-slate-500'
                }`}
              >
                ENEM / VESTIBULAR
            ...
            <div>
              <label className="block text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-2 ml-1">
                {studyType === 'concurso' ? 'Qual concurso você quer passar?' : 'Qual prova você vai fazer?'}
              </label>
              <input 
                type="text" 
                value={exam}
                onChange={(e) => setExam(e.target.value)}
                placeholder={studyType === 'concurso' ? "Ex: Polícia Federal, INSS, Banco do Brasil..." : "Ex: ENEM, FUVEST, Unicamp, Medicina..."}
