...
import { getUserStats, getStudyRoutine } from '../services/db';
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        // Obter tipo de estudo da rotina
        const routine = await getStudyRoutine();
        if (routine?.studyType) {
          setStudyType(routine.studyType as 'concurso' | 'academico');
        }
  ...
  const handleRefreshRadar = async () => {
    ...
    try {
      ...
      const updatedNews = await updateContestRadar(existingTitles, studyType);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 className="text-2xl font-black text-slate-700 dark:text-slate-200 tracking-tight">
                Olá, {studyType === 'academico' ? 'Estudante' : 'Concurseiro'}! 👋
            </h1>
