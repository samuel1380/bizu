...
- **Extração de Vídeo do YouTube (`handleExtractYoutubeContent`)**: 
  - **MODO 1 (Com legendas)**: Se o vídeo tiver legendas/transcrição, ela é extraída e a IA gera uma apostila baseada no conteúdo REAL do vídeo.
  - **MODO 2 (Sem legendas)**: Se NÃO tiver legendas, o sistema pega o título e descrição do vídeo e manda pra IA gerar uma apostila completa sobre o ASSUNTO abordado.
  - A transcrição/título é buscada **ANTES** de escolher a IA (não depende de nenhuma IA para essa etapa).
  - O material é passado para **QUALQUER IA disponível** (Gemini → Mistral → Groq → OpenRouter) via sistema de fallback.
  ...
  - Mensagens de erro agora mostram o MOTIVO REAL da falha (em vez de genérico "Serviço indisponível").
